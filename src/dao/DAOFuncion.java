...
public class DAOFuncion extends DAO
{
	...
	public List<RFC11> rfc11( String localidad, List<String> requerimientosTecnicos, Date hInicio, Date hFin, Date fInicio, Date fEnd ) throws SQLException
	{
		...
		if( requerimientosTecnicos != null && !requerimientosTecnicos.isEmpty( ) )
		{
			sql.append( "AND ( " );
		...
		{
			sql.append( String.format( "AND TO_CHAR( Z.FECHA, 'HH24:MI' ) BETWEEN '%s' AND '%s' ", toTime( hInicio ), toTime( hFin ) ) );
		}
		if( fInicio != null && fEnd != null )
		{
			sql.append( String.format( "AND Z.FECHA BETWEEN %s AND %s ", toDateTime( fInicio ), toDateTime( fEnd ) ) );
		}
