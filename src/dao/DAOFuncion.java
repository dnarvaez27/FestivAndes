...
public class DAOFuncion extends DAO {
	...
	public List<Silla> getSillasDisponibles(Date fechaFuncion, Long idLugarFuncion, Long idLocalidad)
			throws SQLException {
	...
	public List<Funcion> RFC1(String nombreGenero, String nombreCompania, String ciudad, String pais,
			String nombreEspectaculo, String idioma, String fechaInicio, String fechaFin, Integer duracionInicio,
			Integer duracionFin, String lugar, String accesibilidad, String clasificaciones, List<String> order,
		...
		sql.append(nombreGenero != null && !nombreGenero.isEmpty()
				? String.format("AND GENEROS.nombre = '%s' ", nombreGenero) : "");
		sql.append(nombreCompania != null && !nombreCompania.isEmpty()
				? String.format("AND COMPANIAS_DE_TEATRO.nombre = '%s' ", nombreCompania) : "");
		sql.append(pais != null && !pais.isEmpty() ? String.format("AND COMPANIAS_DE_TEATRO.pais_origen = '%s' ", pais)
				: "");
		sql.append(nombreEspectaculo != null && !nombreEspectaculo.isEmpty()
				? String.format("AND ESPECTACULOS.nombre = '%s' ", nombreEspectaculo) : "");
		sql.append(idioma != null && !idioma.isEmpty() ? String.format("AND ESPECTACULOS.idioma = '%s' ", idioma) : "");
		sql.append(duracionInicio != null && duracionFin != null
				? String.format("AND ESPECTACULOS.duracion BETWEEN %s AND %s ", duracionInicio, duracionFin) : "");
		sql.append(lugar != null && !lugar.isEmpty() ? String.format("AND LUGARES.nombre = '%s' ", lugar) : "");
		sql.append(accesibilidad != null && !accesibilidad.isEmpty()
				? String.format("AND ACCESIBILIDADES.nombre = '%s' ", accesibilidad) : "");
		sql.append(clasificaciones != null && !clasificaciones.isEmpty()
				? String.format("AND CLASIFICACIONES.nombre = '%s' ", clasificaciones) : "");
	...
	public boolean validarCancelacion(Date fecha, Long idLugar) throws SQLException {
		...
		Calendar d = SQLUtils.DateUtils.dateToCalendar(fecha);
	...
	public List<RFC11> rfc11(String localidad, List<String> requerimientosTecnicos, Date hInicio, Date hFin,
			Date fInicio, Date fEnd) throws SQLException {
		...
		if (hInicio != null && hFin != null) {
			sql.append(String.format("AND TO_CAR( Z.FECHA, 'HH24:MI' ) BETWEEN %s AND %s ", toTime(hInicio),
					toTime(hFin)));
		}
